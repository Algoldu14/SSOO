...
void eliminar_salto_linea(char *cad)
{
   ...
   longitud = strlen(cad);  /* cad es una cadena de caracteres con la orden introducida por pantalla*/

   for(i = longitud-1; i >= 0; i--) /* se busca el carácter de final de línea introducido por fgets y se sustituye por '\0' para indicar que es el fin de la orden*/
      if (cad[i] == '\n')         
      {
          cad[i] = 0;
          break;
...
void leer_linea_ordenes(char *buf)
{
   ...
   eliminar_salto_linea(buf); //Elimina el salto de linea de las ordenes introducidas
}
